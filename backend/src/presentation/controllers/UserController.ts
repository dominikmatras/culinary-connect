...
import { uploadToS3 } from "../../../utils/uploadToS3AWS";
...
export class UserController {
  ...
  async updateMe(req: Request & { user?: User }, res: Response, next: NextFunction) {
    try {
      ...
      if (req.file) {
        profilePictureUrl = await uploadToS3(req.file, id);
      }
      ...
      const user = await this.userService.updateMe(id, updateData);
